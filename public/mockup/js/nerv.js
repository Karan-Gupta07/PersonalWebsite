...
(function () {
  ...
  function start() {
    ...
    (function initSpotifyWidget() {
      ...
      if (!spotifyText || !navStatus) return;
        ...
        try {
          ...
          if (res.ok) {
            ...
            if (data && (data.title || data.isPlaying)) {
              spotifyText.innerText = "NOW LISTENING TO " + data.title + " - " + (data.artist || "");
              navStatus.setAttribute("data-state", data.isPlaying ? "active" : "idle");
            } else {
              spotifyText.innerText = "STATUS // OFFLINE";
              navStatus.setAttribute("data-state", "idle");
            }
          }
        } catch (err) {
